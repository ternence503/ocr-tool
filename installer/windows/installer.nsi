...
!define MUI_WELCOMEPAGE_TEXT "本工具可將圖片或 PDF 中的文字辨識成可複製的文字，支援繁簡中文、日文、英文。$$
$$
安裝完成後會在桌面建立捷徑，從桌面點兩下即可啟動。$$
$$
⚠ 需要 Python 3.8–3.12（不支援 3.13 以上）$$
若尚未安裝，請先至 python.org 下載 Python 3.12。$$
$$
⏱ 安裝過程需下載套件與辨識模型，約需 5–15 分鐘，請保持網路連線。"

; ── 完成頁文字 ──────────────────────────────────────────────
!define MUI_FINISHPAGE_TITLE "安裝完成"
!define MUI_FINISHPAGE_TEXT "OCR 辨識工具已安裝完成！$$
$$
